...
class LogGenerator(object):
	def __init__(self, endpoint, res_path, *args, **kwargs):
		super(LogGenerator, self).__init__(*args, **kwargs)
		...
		self.set_release_schedule(trace_count, endpoint, 5)
	...
	def modify_sequence(self, trace_count):
		...
		"""
		trace_count['Registration_Triage and Assessment_Intravenous_X-ray_Evaluation_Admission_Discharge']=10
		trace_count['Registration_Triage and Assessment_Intravenous_MRI_Evaluation_Admission_Discharge']=10
		trace_count['Registration_Triage and Assessment_Blood Test_Diagnosis_Admission_Discharge']=10
		trace_count['Registration_Triage and Assessment_Blood Test_Urine Test_Diagnosis_Admission_Discharge']=10
...
if __name__=='__main__':
	#testlog
	Gen = LogGenerator(10*60, res_path="./sample_data/new_resource_0121_1.csv")
	Gen.simulate()
	#show(Gen.p)
	eventlog = Gen.prod_eventlog(start_point='2018-12-01 00:00:00')
	eventlog = Gen.generate_weight(eventlog)
	eventlog.to_csv('../result/testlog_0121_1_40.csv')

	#traininglog
	"""
	Gen = LogGenerator(7*24*60,res_path="./sample_data/new_resource_0121_2.csv")
	Gen.simulate()
	eventlog = Gen.prod_eventlog(start_point='2018-12-01 00:00:00')
	eventlog = Gen.generate_weight(eventlog)
	eventlog.to_csv('../result/traininglog_0121_2.csv')
	"""